...
const PlanCalculator: React.FC = () => {
    ...
    const [result, setResult] = useState<{ totalReturn: number; perReturn: number; totalInvestment: number }>({
        totalReturn: 0,
        perReturn: 0,
        totalInvestment: 0,
    });
    ...
    const calculateReturns = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (hubAmountNum <= 0 || hubCapacityNum <= 0 || durationNum <= 0 || returnPercentageNum <= 0) {
    ...
    const handleMintingTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
